...
for PROJECT in $PROJECTS; do
    ...
    for DOMAIN in $(curl -s --header "PRIVATE-TOKEN: $TOKEN" https://gitlab.com/api/v4/projects/$PROJECT/pages/domains | jq -r .[].domain);do
        ...
        curl -s --request PUT --header "PRIVATE-TOKEN: $TOKEN" --form "certificate=@$LAST_CERT" --form "key=@$LAST_KEY" https://gitlab.com/api/v4/projects/$PROJECT/pages/domains/$DOMAIN
